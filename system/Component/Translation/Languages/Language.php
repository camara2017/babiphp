<?php
    namespace BabiPHP\Component\Translation\Languages;
...
* Main class to convert the plural rules of a language from CLDR to gettext.
		...
		// 		Let's build the category list
            ...
            foreach ($info['categories'] as $cldrCategoryId => $cldrFormulaAndExamples) {
                ...
                foreach ($this->categories as $c) {
                    if ($category->id === $c->id) {
                        throw new Exception("The category '{
			$category->id
		}
	...
	* Let's look for categories that will always occur.
	...
	' should always occur, but it does not have examples (so for CLDR it will never occur for integers!)");
                    }
	...
	' should never occur, but it has some examples (so for CLDR it will occur!)");
                    }
                }
